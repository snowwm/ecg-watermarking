...
class BaseLCBEmbedder(LSBEmbedder, BaseCoder):
    ...
    def embed_chunk(self, wm, coords):
        ...
        for k in range(self.lcb_planes):
            ...
            plane, done = self.embed_plane(
                k, wm[wm_done:], plane, len(coords) * self.lcb_plane_span
            )
            wm_done += done
    ...
    def extract_plane(self, bit_num, max_wm_len, carr):
        comp_start = SIZE_BITNESS * 2
        comp_size = util.bits_to_int(carr[:SIZE_BITNESS], bit_depth=SIZE_BITNESS)
        uncomp_size = util.bits_to_int(
            carr[SIZE_BITNESS:comp_start], bit_depth=SIZE_BITNESS
        )
        wm_start = comp_start + comp_size
        wm_end = wm_start + max_wm_len

        compressed = carr[comp_start:wm_start]
        wm = carr[wm_start:wm_end]

        restored = self.decode(compressed)[:uncomp_size]
    ...
    def _set_uncompressed_plane(self, bit_num, coords, plane):
        mask = (1 << self.lcb_plane_span) - 1
        plane = util.bits_to_ndarray(
            plane, bit_depth=self.lcb_plane_span, dtype=self.restored.dtype
        )
...
class PredMixin:
    ...
    def _set_uncompressed_plane(self, bit_num, coords, plane):
        ...
        diff = util.bits_to_ndarray(
            bits, bit_depth=bit_depth, dtype=self.restored.dtype
        )
