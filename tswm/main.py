...
def make_parser():
    ...
    p = subp.add_parser("info", help="Print some info about EDF(+)/WFDB file(s)")
...
def add_common_args(p):
    ...
    p.add_argument(
        "--pre-trained", action="store_true", dest="_pre_trained", default=None
    )
                ...
                for c in rec.used_channels:
                    try:
                        ...
                        rec.extract_watermark(
                            worker, orig_wm=orig_wm, orig_cont=orig_cont
                        )
...
try:
    ...
    print("❗Warning! Database modifications will not be saved.")
    raise
